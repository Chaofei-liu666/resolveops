...
| 技术项目评审 | 是 | 能说明真实工具调用、审批、验证、故障注入和 Agent 架构取舍 |
...
> ResolveOps 当前达到本地开发、技术项目评审和 ERPNext 沙箱运行级别，已经具备真实业务系统接入、工具调用、Case 状态持久化、调用级审批、结果验证、故障注入和容器化回归测试。它还没有达到真实生产写操作级别，因为生产环境还需要补齐企业 IAM、Secret 管理、监控告警、备份恢复和压测数据。这里把“服务能跑”和“生产可写”分开说明。
