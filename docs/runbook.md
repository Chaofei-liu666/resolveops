...
审批不是永久授权。每条审批会带 `expires_at`，默认由 `APPROVAL_TTL_SECONDS` 控制；过期审批不能继续批准，已经批准但尚未执行的审批在 Worker 执行前也会被重新校验。审批人可调用 `reject-replan` 并提供原因：旧 Plan 的待处理审批会失效，原因与旧 Plan 会进入受限 Case Context，Worker 仅重新执行只读调查，并为新 Plan 创建新的 Approval。需要明确终止时，调用 `revoke`，Case 进入 `manual_review`；即使队列里还残留 execute task，Worker 也会在执行入口阻断写操作。
...
更完整的上线前检查、环境边界、回滚和应急停止流程见 [部署与上线安全清单](deployment.md)。
