...
def test_eval_summary_reports_argument_correctness():
    result = eval_summary_out([
        {
            'resolved': False,
            'manual_review': False,
            'task_succeeded': True,
            'write_invocation_count': 0,
            'tool_call_count': 2,
            'scheduled_tool_call_count': 2,
            'tool_failure_count': 0,
            'llm_call_count': 1,
            'llm_total_tokens': 100,
            'llm_prompt_tokens': 60,
            'llm_completion_tokens': 40,
            'llm_latency_ms_total': 1000,
            'tool_latency_ms_total': 10,
            'observed_tool_latency_count': 2,
            'max_tool_latency_ms': 6,
            'queue_wait_ms': 50,
            'duration_seconds': 2,
            'action_count': 1,
            'argument_correctness': 1,
            'read_tool_budget_used': 0.2,
            'read_tool_budget_exhausted': False,
            'duplicate_tool_call_count': 0,
            'trajectory_quality_score': 1,
            'self_correction_count': 0,
            ...
            'has_replan': False,
            'replan_success': None,
            'has_approval_expired': False,
            'has_approval_revoked': False,
            'has_manual_handoff': False,
        },
        {
            'resolved': False,
    ...
    assert result['argument_checked_cases'] == 2
    assert result['argument_problem_cases'] == 1
    assert result['argument_correctness_rate'] == 0.5
