...
class InvestigationAgent:
    ...
    def _parse_conclusion(content):
        ...
        try:
            ...
            for key in ('alternatives','missing_information','evidence_summary','decision_trace','rejected_actions'):
            ...
            for item in result.get('rejected_actions') or []:
                if isinstance(item,dict):
                    normalized_rejections.append({
                        'action_type': str(item.get('action_type') or item.get('action') or item.get('tool') or 'unknown'),
            ...
            return {
                ...
                'recommended_actions':[],
                'alternatives':[],
                'rationale':'Model conclusion did not meet the required JSON schema.',
                'missing_information':[],
                'evidence_summary':[],
                'decision_trace':[],
                'rejected_actions':[],
